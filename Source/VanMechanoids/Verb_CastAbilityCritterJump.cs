...
{
    public class Verb_CastAbilityCritterJump : Verb_CastAbilityJump
    {
        ...
        protected override bool TryCastShot()
        {
            ...
            CompAbility_CritterJumpTracker jumpTracker = ability.comps.OfType<CompAbility_CritterJumpTracker>().First();
            ...
            foreach (Pawn ally in this.CasterPawn.Map.mapPawns.PawnsInFaction(this.CasterPawn.Faction))
            {
                ...
                if (this.CasterPawn != ally && ally.Spawned && !ally.Downed && !ally.Dead && allyDist <= groupRange * groupRange && ally.abilities.abilities.Any((Ability a) => a.def == VM_DefOf.VM_CritterLongjump))
                {
                    Ability allyAbility = ally.abilities.abilities.FirstOrDefault((Ability a) => a.def == VM_DefOf.VM_CritterLongjump);
                    if (allyAbility != null && allyAbility.CanCast && allyAbility.AICanTargetNow(this.currentTarget))
                    {
                        CompAbility_CritterJumpTracker allyJumpTracker = ability.comps.OfType<CompAbility_CritterJumpTracker>().First();
