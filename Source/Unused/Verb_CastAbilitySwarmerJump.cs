...
namespace VanMechanoids
{
    public class Verb_CastAbilitySwarmerJump : Verb_CastAbilityJump
    {
        ...
        protected override bool TryCastShot()
        {
            ...
            foreach (Pawn ally in CasterPawn.Map.mapPawns.PawnsInFaction(CasterPawn.Faction))
            {
                float allyDist = ally.Position.DistanceToSquared(CasterPawn.Position);
                if (CasterPawn != ally && ally.Spawned && !ally.Downed && !ally.Dead && allyDist <= groupRange * groupRange && ally.abilities.abilities.Any((Ability a) => a.def == VM_DefOf.VM_SwarmerLongjump))
                {
                    Ability allyAbility = ally.abilities.abilities.FirstOrDefault((Ability a) => a.def == VM_DefOf.VM_SwarmerLongjump);
                    if (allyAbility != null && allyAbility.CanCast && allyAbility.AICanTargetNow(currentTarget))
                    {
